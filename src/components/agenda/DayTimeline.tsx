        ...
        {dayApts.map((apt) => {
          ...
          return (
            <div
              ...
              className={cn(
                "absolute left-12 right-1 rounded-lg border px-2 py-1 cursor-pointer overflow-hidden transition-all hover:shadow-md",
                statusColor(apt.status)
              )}
              style={{ top, height, minHeight: SLOT_HEIGHT }}
            >
              <p className="text-xs font-semibold truncate">{apt.cliente_nome}</p>
              <p className="text-[10px] opacity-80 truncate">{apt.servico}</p>
